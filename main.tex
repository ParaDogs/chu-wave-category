...
\begin{theorem}\label{coproduct}
    Пусть $r_i: A_i \times X_i \to D_i$ --- объекты категории $Chu_{\widetilde{Set}}$, $i \in I$. Объект $r: \coprod_{i \in I} A_i \times \prod_{i \in I} X_i \to \coprod_{i \in I} D_i$ категории $Chu_{\widetilde{Set}}$, где $r(a,x) = r_i(a, x_i)$ для любых $i \in I$, $a \in A_i$, $x \in \prod_{i \in I} X_i$, является копроизведением объектов $r_i$, $i \in I$, категории $Chu_{\widetilde{Set}}$.
\end{theorem}
\begin{proof}
    Пусть условия теоремы выполнены и $i \in I$. Определим отображения $q_{A_i}: A_i \to \coprod_{i \in I} A_i$, $p_{X_i}: \prod_{i \in I} X_i \to X_i$, $q_{D_i}: D_i \to \coprod_{i \in I} D_i$ следующим образом: $q_{A_i}(a) = a$, $p_{X_i}(x) = x_i$, $q_{D_i}(d) = d$ для любых $a \in A_i$, $x \in \prod_{i \in I} X_i$, $d \in D_i$. Поскольку $q_{D_i}(r_i(a,p_{X_i}(x))) = r_i(a,x_i) = r(a,x) = r(q_{A_i}(a),x)$ для любых $a \in A_i$, $x \in \prod_{i \in I} X_i$, то $(q_{A_i}, p_{X_i}, q_{D_i})$ --- преобразование Чу категории $Chu_{\widetilde{Set}}$.
...
\begin{theorem}\label{product}
    Пусть $r_i: A_i \times X_i \to D_i$ --- объекты категории $Chu_{\widetilde{Set}}$, $i \in I$. Объект $r: \prod_{i \in I} A_i \times \coprod_{i \in I} X_i \to \prod_{i \in I} D_i$ категории $Chu_{\widetilde{Set}}$, где $r(a,x)_i = r_i(a_i, x)$ для любых $i \in I$, $a \in \prod_{i \in I} A_i$, $x \in X_i$, является произведением объектов $r_i$, $i \in I$, категории $Chu_{\widetilde{Set}}$.
\end{theorem}
\begin{proof}
    Пусть условия теоремы выполнены и $i \in I$. Определим отображения $p_{A_i}: \prod_{i \in I} A_i \to A_i$, $q_{X_i}: X_i \to \coprod_{i \in I} X_i$, $p_{D_i}: \prod_{i \in I} D_i \to D_i$ следующим образом: $p_{A_i}(a) = a_i$, $q_{X_i}(x) = x$, $p_{D_i}(d) = d_i$ для любых $a \in \prod_{i \in I} A_i$, $x \in X_i$, $d \in \prod_{i \in I} D_i$. Поскольку $p_{D_i}(r(a,q_{X_i}(x))) = r_i(a_i,x) = r_i(p_{A_i}(a),x)$ для любых $a \in \prod_{i \in I} A_i$, $x \in X_i$, то $(p_{A_i}, q_{X_i}, p_{D_i})$ --- преобразование Чу категории $Chu_{\widetilde{Set}}$.

    Пусть $t: B \times Y \to D$ --- объект категории $Chu_{\widetilde{Set}}$, $(f_i,g_i,h_i): t \to r_i$ --- преобразование Чу категории $Chu_{\widetilde{Set}}$ для любого $i \in I$. Так как $\prod_{i \in I} A_i$ и $\prod_{i \in I} D_i$ --- произведения $A_i$, $ i \in I$, и $D_i$, $i \in I$, соответственно, а $\coprod_{i \in I} X_i$ --- копроизведение $A_i$, $i \in I$ в категории $Set_{\star}$, то существуют единственные морфизмы $\tilde{f}: B \to \prod_{i \in I} A_i$, $\tilde{g}: \coprod_{i \in I} X_i \to Y$, $\tilde{h}: D \to \prod_{i \in I} D_i$ категории $Set_{\star}$ такие, что $f_i = p_{A_i} \circ \tilde{f}$, $g_i = \tilde{g} \circ q_{X_i}$ и $h_i = p_{D_i} \circ \tilde{h}$ для любого $i \in I$, то есть $\tilde{f}(b)_i = f_i(b)$, $\tilde{g}(x) = g_i(x)$ и $\tilde{h}(d)_i = h_i(d)$ для любых $b \in  B$, $x \in X_i$, $d \in D$. Следовательно, $(p_{A_i}, q_{X_i}, p_{D_i}) \circ (\tilde{f},\tilde{g},\tilde{h}) = (f_i, g_i, h_i)$ для любого $i \in I$ в категории $Chu_{\widetilde{Set}}$. УТОЧНИТЬ.

    Покажем, что $(\tilde{f},\tilde{g},\tilde{h})$ --- преобразование Чу категории $Chu_{\widetilde{Set}}$, то есть для любых $b \in B$, $x \in X_i$ имеет место равенство $\tilde{h}(t(b,\tilde{g}(x))) = r(\tilde{f}(b),x)$. Так как $(p_{A_i},q_{X_i},p_{D_i})$ --- преобразование Чу категории $Chu_{\widetilde{Set}}$, то $r_i(p_{A_i}(a),x) = p_{D_i}(r(a,q_{X_i}(x)))$ для любых $a \in \prod_{i \in I} A_i$, $x \in X_i$. Из того, что $h_i = p_{D_i} \circ \tilde{h}$ и $g_i = \tilde{g} \circ q_{X_i}$ следует, что $p_{D_i}(\tilde{h}(t(b,\tilde{g}(x)))) = h_i(t(b,g_i(x)))$ для любых $a \in \prod_{i \in I} A_i$, $x \in X_i$. Так как $(f_i,g_i,h_i)$ --- преобразование Чу категории $Chu_{\widetilde{Set}}$, то $h_i(t(b,g_i(x))) = r(f_i(b),x)$ для любых $a \in \prod_{i \in I} A_i$, $x \in X$. Поскольку $\tilde{f}(a) = f_i(a)$, то $t(f_i(a),y) = t(\tilde{f}(a),y)$ для любых $a \in A_i$, $y \in Y$. Таким образом, для любых $a \in \coprod_{i \in I} A_i$, $y \in Y$ имеет место равенство $\tilde{h}(t(b,\tilde{g}(x))) = r(\tilde{f}(b),x)$.
\end{proof}
