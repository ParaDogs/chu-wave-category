...
\section*{Категория $Chu_\sim(C)$}

\begin{theorem}\label{epimorphism-c}
    Пусть $r: A \wedge X \to D$ и $t: B \wedge Y \to C$ --- объекты категории $Chu_\sim(C)$. Преобразование $(f,g,\h): r \to t$ категории $Chu_\sim(C)$ является эпиморфизмом тогда и только тогда, когда $f: A \to B$ --- эпиморфизм, $g: Y \to X$ --- мономорфизм категории $C_{\star}$.
\end{theorem}
\begin{proof}
    \textbf{Необходимость.} Пусть $(f,g,\h): r \to t$ --- эпиморфизм категории $Chu_\sim(C)$.

    Покажем, что $f$ --- эпиморфизм категории $C$. Предположим, что $B_1 \ne B$, где $B_1 = f(A)$. Через $B_0$ обозначим факторсистему алгебраической системы $B$ по конгруэнции Риса $\sim$ такой, что $b \sim b' \Leftrightarrow b, b' \in B_1$. Определим объект $w: (B_0 \times B) \wedge Y \to C$ и морфизмы $(f_1,1_Y,1_C), (f_2,1_Y,1_C): t \to w$ категории $Chu_\sim(C)$ следующим образом: $w((b_0,b),y) = t(b,y)$, $f_1(b) = (B_1,b)$, $f_2(b) = (b/\sim,b)$ для любых $b \in B$, $b_0 \in B_0$, $y \in Y$. Корректность определения морфизмов $(f_1,1_Y,1_C)$, $(f_2,1_Y,1_C)$ следует из равенств:
    $$
    ...
    Покажем, что $g$ --- мономорфизм категории $C$. Предположим, что существуют различные $y_1, y_2 \in Y$ такие, что $g(y_1) = g(y_2)$. Пусть $b \in B$. Покажем, что $t(b,y_1) = t(b,y_2)$. Так как $f$ --- эпиморфизм, то существует $a \in A$ такой, что $f(a) = b$. Тогда
    $$
    ...
    \textbf{Достаточность.} Пусть $(f,g,\h): r \to t$ --- преобразование Чу категории $Chu_\sim(C)$, где $f$ --- эпиморфизм, $g$ --- мономорфизм категории $C_{\star}$. Предположим, что $(f_1,g_1,h_1), (f_2,g_2,h_2): t \to w$ --- преобразования Чу категории $Chu_\sim(C)$ такие, что $(f_1,g_1,h_1) \circ (f,g,\h) = (f_2,g_2,h_2) \circ (f,g,\h)$, где $w: E \wedge Z \to P$ --- пространство Чу категории $Chu_\sim(C)$. Тогда $f_1 \circ f = f_2 \circ f$, $g \circ g_1 = g \circ g_2$ и для любого $d \in D$ если $d = r(a,(g \circ g_1)(z)) = r(a,(g \circ g_2)(z))$, то $(h_1 \circ h)(d) = (h_2 \circ h)(d) = w((f_1 \circ f)(a), z) = w((f_2 \circ f)(a), z)$. Покажем, что для любого $c \in C$ если $c = t(b,g_1(z))$, то $h_1(c) = h_2(c) = w(f_1(b),z)$. Пусть $c \in C$ и $c = t(b,g_1(z))$ для некоторых $b \in B$, $z \in Z$. Поскольку $f$ --- эпиморфизм, $g$ --- мономорфизм категории $C$, то $f_1 = f_2$, $g_1 = g_2$. Так как $f$ --- эпиморфизм, то $b = f(a)$ для некоторого $a \in A$. Тогда 
    \begin{multline*}
