...
\begin{proof}
    ...
    \end{cases}$ Определим объект $w: (B_0 \times A_2) \wedge X_2 \to D_2$ и морфизмы $(f_1,e_{X_2},\widetilde{e_{D_2}}), (f_2,e_{X_2},\widetilde{e_{D_2}}): r_2 \to w$ категории $Chu_\sim(C_\star)$ следующим образом: $w((b_0,a),x) = r_2(a,x)$, $f_1(a) = (B_1,a)$, $f_2(a) = (a/\theta,a)$ для любых $a \in A_2$, $b_0 \in B_0$, $x \in X_2$. Корректность определения морфизмов $(f_1,e_{X_2},\widetilde{e_{D_2}})$, $(f_2,e_{X_2},\widetilde{e_{D_2}})$ следует из равенств:
    $$
        r_2(a,x) = w(f_1(a),x) = w(f_2(a),x)
    $$
    для любых $a \in A_2$, $x \in X_2$. Если $a \in A_1$, то $f^+(a) \in B_1$ и $f_1(f^+(a)) = f_2(f^+(a))$, то есть $f_1 \circ f^+ = f_2 \circ f^+$. Тогда $(f_1,e_{X_2},\widetilde{e_{D_2}}) \circ (f^+,f^-,\fo) = (f_2,e_{X_2},\widetilde{e_{D_2}}) \circ (f^+,f^-,\fo)$. Поскольку $(f^+,f^-,\fo)$ --- эпиморфизм категории $Chu_\sim(C_\star)$, то $f_1 = f_2$. Противоречие.
